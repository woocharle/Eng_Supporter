		...
		for (HVO2 k : list) {
			if(k.getIdx().equals(idx)) {
				...
				try {
					...
					double flow = change(k.getFlow());
					double din = change(k.getDin());
					double dia = change(k.getDia()) * 25.4;
					
					if (k.getPhase().equals("liquid")) {
						flow_liq = flow;
					}else if (k.getPhase().equals("vapor")) {
						flow_vap = flow;
					}else {
						flow_vap = flow * vapor_fr;
						flow_liq = flow - flow_vap;
					}
					
					k.setFlow_liq(request.getParameter("flow_liq".concat(idx)));
					k.setFlow_vapor(request.getParameter("flow_vapor".concat(idx)));
					
					double re = pipespec.calRe(k.getPhase(), den_liq, vis_liq, flow_liq, den_vap, vis_vap, flow_vap, din);
					k.setRe(request.getParameter("re".concat(k.getIdx())));
					double eqv_ld = 0;
						...
						for (int i = 0; i < 38; i++) {
							if (fitting_n[i] != 0 ) {
								if (i == 35) {
									...
									eqv_ld += fitting_n[i] * fitting_cal[i][3]; 
								}
